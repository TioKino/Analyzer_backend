...
def analyze_structure_and_cues(y: np.ndarray, sr: int, duration: float,
    ...
    try:
        logger.info(f"Analisis preciso v3: {duration:.1f}s, BPM={bpm}")
        
        # 1. Beat grid
        beat_times, first_beat, beat_interval = get_beat_times(y, sr, bpm)
        logger.debug(f"Beat grid: {len(beat_times)} beats, interval={beat_interval:.4f}s")
        
        # 2. Estructura precisa
        structure = detect_structure_precise(y, sr, bpm, beat_times)
        sections = structure['sections']
        logger.debug(f"Estructura: {len(sections)} secciones")
        for s in sections:
            logger.debug(f"  {s['type']:>10} | {s['start']:6.1f}s - {s['end']:6.1f}s | E={s['energy']:.2f}")
        
        # 3. Cue points beat-aligned
        cue_points = detect_cue_points_precise(y, sr, duration, sections, beat_times, bpm)
        logger.debug(f"Cue points: {len(cue_points)}")
        for cp in cue_points:
            logger.debug(f"  {cp['type']:>15} | {cp['timestamp']:6.1f}s | conf={cp['confidence']:.2f}")
        ...
        logger.error(f"Error en analisis preciso: {e}")
        import traceback
        traceback.print_exc()
        return _fallback_result(duration)
